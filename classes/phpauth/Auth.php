...
class Auth
{
  ...
  */
  ...
  // By Kjetil
  public function third_party_login($user_id, $remember = 0)
  {
    $return['error'] = true;
    
    $tbl = get_object_table('user');
    $client_id = get_object('user', $user_id, "{$tbl}.client_id");
    if($client_id != $this->config->client_id) {
      $return['message'] = $this->lang["system_error"] . " #16";
      return $return;
    }
      
    $sessiondata = $this->addSession($user_id, $remember);
    ...
    $return['hash'] = $hash = $sessiondata['hash'];
    $return['expire'] = $expire = $sessiondata['expiretime'];
    
    if($expire) $this->setSessionHash($hash, $expire); // by Kjetil    
    update_object("user", ["last_login" => now()], $user_id); // by Kjetil
    return $return;
  }
  ...
  */
  ...
  {
    global $__SITE, $__CLIENT_ID;
    $site_id = $this->config->site_id ?: $__SITE->id;
    $client_id = $this->config->client_id ?: $__CLIENT_ID;
    
    print_log("login username={$email} site_id={$site_id} client_id={$client_id}", 'auth', LOG_LEVEL_ALWAYS);
    ...
    if($block_status == "verify")
    {
      if($this->checkCaptcha($captcha) == false)
      {
        ...
        $return['status'] = LOGIN_WRONG_CAPTCHA;
        return $return;
      }
    }
    if ($block_status == "block") {
      $return['message'] = $this->lang["user_blocked"];
      $return['status'] = LOGIN_USER_BLOCKED;
      return $return;
    }
    ...
    if ($validateEmail['error'] == 1) {
      ...
      $return['status'] = LOGIN_WRONG_EMAIL;
      return $return;
      ...
      $this->addAttempt();
      $return['status'] = LOGIN_INVALID_REMEMBER;
      $return['message'] = $this->lang["remember_me_invalid"];
      return $return;
    }

    $uid = $this->getUID(strtolower($email), $client_id);

    if(!$uid) {
      $this->addAttempt();

      $return['status'] = LOGIN_INVALID_USER;
    ...
    $return['user_id'] = $uid;
    $tbl = get_object_table('user');
    $client_id = get_object('user', $uid, "{$tbl}.client_id");
    if($uid != SYSTEM_USER_ID && $client_id != $this->config->client_id) {
      $return['status'] = LOGIN_INVALID_CLIENT_ID;
      $return['message'] = $this->lang["system_error"] . " #16";
      return $return;
    }
    ...
    if (!password_verify($password, $user['password'])) {
      ...
      $return['status'] = LOGIN_INVALID_PASSWORD;
      $return['message'] = $this->lang["email_password_incorrect"];
      return $return;
    }
    ...
    if ($user['active'] != 1) {
      ...
      $return['status'] = LOGIN_INACTIVE_ACCOUNT;
    ...
    if($sessiondata == false) {
      $return['status'] = LOGIN_INVALID_SESSION;
      $return['message'] = $this->lang["system_error"] . " #01";
      return $return;
    }
    ...
    if($expire) $this->setSessionHash($hash, $expire); // by Kjetil
    update_object("user", ["last_login" => now()], $uid); // by Kjetil
  ...
  */
  ...
  public function register($email, $password, $repeatpassword, $params = Array(), $captcha = NULL, $sendmail = NULL)
  {
    ...
    $block_status = $this->isBlocked();
    $validate_email = isset($params['validate_email']) ? $params['validate_email'] : true;
    $site_id = isset($params['site_id']) ? $params['site_id'] : $__SITE->id;
    $client_id = isset($params['client_id']) ? $params['client_id'] : $__CLIENT_ID;

    
    print_log("register username={$username} site_id={$site_id} client_id={$client_id}", 'auth', LOG_LEVEL_ALWAYS);
    unset($params['validate_email']);
    ...
    //$return['message'] = "foo4: email: $email pw: $password repeat: $repeatpassword";
    //return $return;
  ...
  */
  ...
  public function activate($key)
  {
    ...
    if($this->getBaseUser($getRequest['uid'])['active'] == 1) {
      $this->addAttempt();
    ...
    $query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET active = ? WHERE id = ?");
  ...
  */
  ...
  public function getUID($email, $client_id)
  {
    $query = $this->dbh->prepare("SELECT id FROM {$this->config->table_users} WHERE username = ? AND client_id = ?");
    $query->execute(array($email, $client_id));
  ...
  */
  ...
  public function isEmailTaken($email, $client_id)
  {
    $query = $this->dbh->prepare("SELECT count(*) FROM {$this->config->table_users} WHERE username = ? AND client_id = ?");
    $query->execute(array($email, $client_id));
  ...
  */
  ...
  private function addUser($email, $password, $params = array(), &$sendmail)
  {
    ...
    $query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET username = ?, password = ?, active = ?, site_id = ?, client_id = ? {$setParams} WHERE id = ?");

    if(!$params || !is_array($params)) $params = []; // kjetil
    $bindParams = array_values(array_merge(array($email, $password, $active, $this->config->site_id, $this->config->client_id), $params, array($uid)));
    ...
    db_query("UPDATE db_sequence SET nextid=$uid WHERE seq_name = '{$this->config->table_users}'");
    $return['error'] = false;
    $return['id'] = $uid; // Kjetil
    return $return;
  }
  ...
  */
  ...
  {
    $query = $this->dbh->prepare("SELECT username, password, active FROM {$this->config->table_users} WHERE id = ?");
    $query->execute(array($uid));
  ...
  */
  ...
  private function addRequest($uid, $email, $type, &$sendmail)
  {
    ...
    if($type == "activation" && $this->getBaseUser($uid)['active'] == 1) {
    ...
    if($sendmail === true) { // By Kjetil - use SwiftMail
      ...
      if($type == "activation") {
        ...
        $body = sprintf($this->lang['email_activation_body'], $this->config->site_url, $this->config->site_activation_page, $key);
        //$body .= html_break(2).mail_footer($_SESSION['siteObj'], $_SESSION['language']);
        $altBody = sprintf($this->lang['email_activation_altbody'], $this->config->site_url, $this->config->site_activation_page, $key);
      } else {
        $subject = sprintf($this->lang['email_reset_subject'], $this->config->site_name);
        $body = sprintf($this->lang['email_reset_body'], $this->config->site_url, $this->config->site_password_reset_page."?guid={$key}", $key);
        $altBody = sprintf($this->lang['email_reset_altbody'], $this->config->site_url, $this->config->site_password_reset_page."?guid={$key}", $key);
      }

      $link = html_link($url, $title ?: $url);

      $mail_scope = ['main_message' => $body];
      $body = smarty_mail_template($siteObj, 'mail/mail-generic', ['scope' => $mail_scope]);
      $mail_options = ["sender" => $from, "parent_type" => 'user', "parent_id" => $uid];
      $result = send_mail($site_id, $subject, $body, $to, $mail_options);
      
      if(!$result->success) {
        $this->deleteRequest($request_id);
        $return['message'] = $result->error ?: $this->lang["system_error"] . " #10";
        return $return;
      }
  ...
  */
  ...
  private function validateEmail($email) {
    ...
    if(0 && !in_array($email, ['anke', 'siccoadmin', 'system', 'canalapartments', 'trulove', 'uhuru', 'kj', 'KochxBos', 'rolf', 'kerry'])) { // Kjetil: allow old users
  ...
  /**
  * Check if reset key is valid (Kjetil).
  * @param string $key
  * @return array $return
  */
  ...
  */
  ...
  public function resendActivation($email)
  {
    ...
    if ($this->getBaseUser($row['id'])['active'] == 1) {
      $this->addAttempt();
  ...
  */
  public function changePassword($uid, $currpass, $newpass, $repeatnewpass, $captcha = NULL)
  {
    ...
    if($validatePassword['error'] == 1) {
      $this->addAttempt();
      //print_log("Auth no validate password".dump($validatePassword, true), 'auth', LOG_LEVEL_ALWAYS); 
      $return['message'] = $validatePassword['message'] ?: "Please use at least one uppercase letter, one lowercase letter, and one number";
      return $return;
    }
  ...
  */
  ...
  /**
   * Set the session hash (by Kjetil)
   * @return string
   */
  public function setSessionHash($hash, $expire) {
    return setcookie($this->config->cookie_name, $hash, (int) $expire, '/');
  }
